...
export class CoralGenerator {
  ...
  constructor() {
    ...
    this.attractorShape = new SamplingShape(AttractorShape.Sphere, () => {
      this.Reset()
    })
    this.init()
  }
  ...
  GenerateMeshFromVerts() {
    let vertices = new Float32Array(
      (this.branches.length + 1) * this.radialSegments * 3
    )
    let indices = new Uint32Array(
      this.branches.length * this.radialSegments * 6
    )
    ...
    for (let i = 0; i < this.branches.length; i++) {
      ...
      let bid =
        this.branches[i].parent != null
          ? this.branches[i].parent.verticesId
          : this.branches.length * this.radialSegments
  ...
  CreateGUI(gui: lil.GUI) {
    ...
    attractorFolder
      .add(this, 'attractorCount', 0, 10000, 1)
      .name('Count')
      .onChange(() => {
        this.Reset()
      })
    attractorFolder
      .add(this, 'attractorRadius', 0, 10, 0.1)
      .name('Sample Radius')
      .onChange(() => {
        this.Reset()
      })
    attractorFolder
      .add(this, 'attractorStrength', 0, 10, 0.1)
      .name('Influence Radius')
      .onChange(() => {
        this.Reset()
      })
    attractorFolder
      .add(this, 'attractorKillRange', 0, 10, 0.1)
      .name('Kill Radius')
      .onChange(() => {
        this.Reset()
      })
    attractorFolder
      .add(this, 'attractorFood', 0, 2, 0.1)
      .name('Food')
      .onChange(() => {
        this.Reset()
      })
    attractorFolder
      .add(this.attractorShape, 'shapeType', {
        Hemisphere: AttractorShape.Hemisphere,
        Sphere: AttractorShape.Sphere,
        Cuboid: AttractorShape.Cuboid,
        Cone: AttractorShape.Cone,
        Cylinder: AttractorShape.Cylinder,
      })
      .name('Shape')
      .onChange(() => {
        this.Reset()
